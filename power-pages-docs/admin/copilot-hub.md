...
|----------------------------------|---------------------------------------------------------------------------------------|
| [Chat agent or add an agent](../getting-started/enable-agent.md)        | <ul><li> **Total Sites with Chat agent Enabled**: Total number of sites where the chat agent feature is enabled. </li><li> **Usage Table & Trend View**: Displays usage patterns over time and includes a detailed table with information such as site name, monthly active users (MAU), site type, and environment. </li><li> **Monthly Active Users (MAU)**: Number of end users who received at least one successful AI-generated response from the Site Copilot chatbot during the selected time period. </li></ul> |
