...
Power Pages addresses the challenge of defining site settings values across different environments with the enhanced data model. While the standard data model uses deployment profiles, the enhanced model allows for dynamic configuration of environment-specific settings using environment variables.

## Key definitions

Understanding the following key definitions is important for comprehending how environment variables can be utilized with site settings:
...
Using environment variables for site settings offers several benefits, including:
...
1. Open the [Power Pages management application](portal-management-app.md) and go to **Site Settings**.
...
> - Data type set to "data source" isn't supported.
...
During the import process into the target environment, assign a value to the environment variable. Modifying the value updates the corresponding site settings in the target environment.
...
- **Option 1:** Select **Sync** in the [design studio](../getting-started/use-design-studio.md).
...
The following are some different reasons you may encounter errors when using environment variables with site settings:
...
- **Usage recommendation:** Use this feature only for ALM-related settings or secrets. Avoid using it for all site settings because it can lead to performance degradation.
- **Name consistently:** Use consistent names for environment variables to make them easy to identify.
- **Document variables:** Maintain a reference document listing all environment variables, their purposes, and their default values.
- **Test thoroughly:** Validate changes in a nonproduction environment before applying them to production.
- **Use descriptions:** Use the description field to explain the variable's purpose and usage.

## Configure environment variables as secrets

To configure environment variables as secrets and use them in Power Pages:

1. Configure an environment variable as a secret: [Use environment variables for Azure Key Vault secrets](/power-apps/maker/data-platform/environmentvariables-azure-key-vault-secrets)
...
1. **Can I edit environment variable values directly in production?**  
    Yes, but make sure changes are tested and documented to avoid unintended impacts.
