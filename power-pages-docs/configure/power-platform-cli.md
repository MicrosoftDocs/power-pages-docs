...
> [!NOTE]
> **When you download a portal from *Environment A* and later upload it into *Environment B*, PAC CLI always performs a *full* upload.**  
> The target environment has no previous state to compare against, so **delta detection is impossible**.  
> Only when you *download* and *upload* within the *same* environment can PAC CLI local file changes and only upload updated files.
> Recommended when to use `--forceUploadAll`. (This parameter is currently CLI-only. The Azure DevOps asks do *not* surface it yet.)
> - *Cross-environment moves* (you exported from a sandbox but will publish to prod).  
