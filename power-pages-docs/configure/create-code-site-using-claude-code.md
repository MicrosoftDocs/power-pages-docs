...
description: This page provides a walk-through on how to create, customize, and deploy single-page applications for Microsoft Power Pages using an agentic AI coding tool.
...
> [!IMPORTANT]
> - A generative AI coding agent, such as GitHub Copilot CLI or Claude Code, uses this plugin's skills and workflows to complete Power Pages-specific tasks.
> - Code and configuration generated by the AI coding agent can be inaccurate. Review, test, and validate all generated content before you approve or deploy it. Always [review agent proposals before approving](#review-agent-proposals-before-approving) them.
...
| Add AI summaries | `/add-ai-webapi` | Integrates the generative AI summarization APIs (search summary and data summarization) into your pages. Generates a typed summary service with anti-forgery token handling, framework-specific wrappers, wired call sites, and the matching `Summarization` site settings |
...
> - To add AI-generated search and record summaries, run `/add-ai-webapi` after `/integrate-webapi`. It reuses the table permissions and site settings that `/integrate-webapi` configures, so run it once your Web API integration is in place.
...
#### Add AI-generated summaries (Optional)

Run `/add-ai-webapi` to layer generative AI summaries on top of your Web API integration. The skill connects two preview summarization APIs to your record-detail and list pages:

- **Search summary API**: Returns an AI-condensed summary of search results, with citations.
- **Data summarization API**: Returns a per-record summary or list-level insights for a Dataverse table.

This step builds on the data model from Step 4 and the Web API integration from Step 6. The plugin performs the following actions:

1. Scans your code for summary integration points and confirms the target pages, tables, and which APIs to use.
1. Delegates any missing Web API prerequisites—table permissions, column lists, and web roles—to `/integrate-webapi` and `/create-webroles`, so there's a single source of truth for permissions.
1. Spawns an **AI Web API Integration** agent that generates a typed summary service (`aiSummaryService.ts`) with anti-forgery token handling, framework-specific wrappers (React hooks, Vue composables, or Angular services), and wired call sites on the target pages.
1. Spawns an **AI Web API Settings Architect** agent that creates the `Summarization` site settings for prompts and content limits.

**You review and approve the proposals.** The plugin doesn't generate code or create settings until you confirm.

> [!NOTE]
> The summarization APIs are in preview and depend on administrative and site-level controls being turned on. If summaries don't appear after deployment, confirm that the feature is enabled through the relevant site settings and admin controls. For more information, see [Data summarization API overview](data-summarization-api.md) and [Power Pages search with generative AI](search/generative-AI.md).
...
If you choose the manual path, the plugin runs:
...
The AI coding agent presents proposals, such as those from the Data Model Architect and Permissions Architect, before it makes changes. Code and configuration generated by the AI coding agent can be inaccurate, so review, test, and validate every proposal and all generated code before you approve or deploy it.
...
- **Generated code**: Read through the generated code and test the site locally before you deploy. AI-generated code can contain errors or security issues.
...
- [Data summarization API overview](data-summarization-api.md)
- [Power Pages search with generative AI](search/generative-AI.md)
